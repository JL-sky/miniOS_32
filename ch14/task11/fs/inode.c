...
{
    // inode_table在硬盘上是连续的，因此有可能会跨越扇区存储
    // 故该字段是为了标记inode是否跨越扇区
    bool two_sec;
    // inode所在扇区号
    uint32_t sec_lba;
    // inode所在扇区的字节偏移
...
/*
将内存中的 inode 数据同步到硬盘中
参数：
    - part：指向分区的指针，包含分区的起始 LBA（逻辑块地址）和扇区数量等信息。
    - inode：指向内存中 inode 结构的指针，表示需要同步的 inode。
    - io_buf：用于硬盘 I/O 操作的缓冲区。
*/
void inode_sync(struct partition *part, struct inode *inode, void *io_buf)
{
    ...
    pure_inode.inode_tag.prev = pure_inode.inode_tag.next;
...
{
    /*先在内存已打开的inode链表中查找inode，此链表是为提速创建的缓冲区*/
